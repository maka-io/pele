 ...
 */
const insertObservation = new ValidatedMethod({
    name: 'observation.insert',
    validate: null,
    run(doc) {
        //return Observation.insert(doc);
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const updateObservation  = new ValidatedMethod({
    name: 'observation.update',
    validate: null,
    run([docId, obj]) {
        //return Observation.update(docId, {$set: obj});
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const removeObservation  = new ValidatedMethod({
    name: 'observation.remove',
    validate: null,
    run(docId) {
        //return Observation.remove(docId);
    }
});

const RATE_LIMITED_METHODS = _.pluck([
    insertObservation , updateObservation , removeObservation
], 'name');
