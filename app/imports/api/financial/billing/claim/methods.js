 ...
 */
const insertClaim = new ValidatedMethod({
    name: 'claim.insert',
    validate: null,
    run(doc) {
        //return Claim.insert(doc);
    }
});
...
const updateClaim  = new ValidatedMethod({
    ...
    validate: null,
    run([docId, obj]) {
        //return Claim.update(docId, {$set: obj});
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const removeClaim  = new ValidatedMethod({
    name: 'claim.remove',
    validate: null,
    run(docId) {
        //return Claim.remove(docId);
    }
});

const RATE_LIMITED_METHODS = _.pluck([
    insertClaim , updateClaim , removeClaim
], 'name');
