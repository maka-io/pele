 ...
 */
const insertCommunicationRequest = new ValidatedMethod({
    name: 'communication-request.insert',
    validate: null,
    run(doc) {
        //return CommunicationRequest.insert(doc);
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const updateCommunicationRequest  = new ValidatedMethod({
    name: 'communication-request.update',
    validate: null,
    run([docId, obj]) {
        //return CommunicationRequest.update(docId, {$set: obj});
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const removeCommunicationRequest  = new ValidatedMethod({
    name: 'communication-request.remove',
    validate: null,
    run(docId) {
        //return CommunicationRequest.remove(docId);
    }
});

const RATE_LIMITED_METHODS = _.pluck([
    insertCommunicationRequest , updateCommunicationRequest , removeCommunicationRequest
], 'name');
