 ...
 */
const insertPerson = new ValidatedMethod({
    name: 'person.insert',
    validate: null,
    run(doc) {
        //return Person.insert(doc);
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const updatePerson  = new ValidatedMethod({
    name: 'person.update',
    validate: null,
    run([docId, obj]) {
        //return Person.update(docId, {$set: obj});
    }
});
 ...
 * @property { function }   run         The main action that is executed.
 */
const removePerson  = new ValidatedMethod({
    name: 'person.remove',
    validate: null,
    run(docId) {
        //return Person.remove(docId);
    }
});

const RATE_LIMITED_METHODS = _.pluck([
    insertPerson , updatePerson , removePerson
], 'name');
